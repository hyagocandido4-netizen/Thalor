...
AUTO_VOLUME_PY = r'''# P12f: auto volume controller (safe defaults from P14 sweep)
from __future__ import annotations
...
def patch_wrapper_sync_cp_alpha(root: Path) -> None:
    ps1 = root / "scripts" / "scheduler" / "observe_loop_auto.ps1"
    if not ps1.exists():
        print("[P12f] wrapper não encontrado, pulando:", ps1)
        return

    marker = "# P12f: sync CP_ALPHA with CPREG_ALPHA_END"
    txt = ps1.read_text(encoding="utf-8", errors="replace")
    if marker in txt:
        print("[P12f] wrapper já tem sync CP_ALPHA.")
        return
    ...
    for line in lines:
        ...
        # after applying rec.cpreg_alpha_end
        if ("CPREG_ALPHA_END" in line) and ("rec.cpreg_alpha_end" in line) and ("$env:" in line):
            out.append(marker + "\n")
            out.append("$env:CP_ALPHA = $env:CPREG_ALPHA_END\n")
            inserted_any = True

        # after seeding pr.cpreg_alpha_end
        if ("CPREG_ALPHA_END" in line) and ("pr.cpreg_alpha_end" in line) and ("$env:" in line):
            out.append(marker + "\n")
            out.append("$env:CP_ALPHA = $env:CPREG_ALPHA_END\n")
            inserted_any = True

    if not inserted_any:
        print("[P12f] Não achei linhas CPREG_ALPHA_END no wrapper para inserir CP_ALPHA. (não é fatal)")
        return

    bkp = backup_if_exists(ps1)
    ps1.write_text("".join(out), encoding="utf-8")
    print(f"[P12f] OK wrapper patched: {ps1} (backup={bkp})")
...
def main() -> None:
    ...
    bkp = backup_if_exists(auto_p)
    auto_p.write_text(AUTO_VOLUME_PY, encoding="utf-8")
    py_compile.compile(str(auto_p), doraise=True)
    print(f"[P12f] OK wrote {auto_p} (backup={bkp})")

    patch_wrapper_sync_cp_alpha(root)
