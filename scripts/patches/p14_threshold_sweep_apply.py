...
# IMPORTANTE: usamos aspas triplas simples aqui (''') para permitir docstrings """ dentro do módulo.
SWEEP_MODULE = r'''# P14: threshold/cp_alpha sweep for paper_pnl_backtest (black-box)
from __future__ import annotations
...
def main() -> None:
    ...
    if args.cp_alphas.strip():
        ...
        alpha_list = [float(os.getenv("CP_ALPHA", "0.07"))]
...
def main() -> None:
    root = repo_root()
    mod = root / "src" / "natbin" / "sweep_thresholds.py"
    mod.parent.mkdir(parents=True, exist_ok=True)

    backup_if_exists(mod)
    mod.write_text(SWEEP_MODULE, encoding="utf-8")
    py_compile.compile(str(mod), doraise=True)

    print(f"[P14] OK wrote {mod}")
