...
"""
P17b - Fix apply script for hour-aware threshold.

Fixes:
- Previous p17_hourthr_apply.py used an f-string to build the code for auto_hourthr.py,
  and the generated code contains inner f-strings like f"{hour}:00".
  The outer f-string tried to evaluate {hour} at patch-time -> NameError.

This patch is idempotent:
- Writes src/natbin/auto_hourthr.py if missing (or missing marker).
- Patches scripts/scheduler/observe_loop_auto.ps1 inserting a safe block after P16,
  without breaking any multi-line "-f" expressions.
"""
...
def write_auto_hourthr(py_path: Path) -> None:
    if py_path.exists():
        ...
        if PY_MARKER in txt:
            print(f"[P17b] auto_hourthr já existe: {py_path}")
            return

    py_path.parent.mkdir(parents=True, exist_ok=True)

    # IMPORTANT: do NOT use f-string here; the generated file contains f"{...}".
    code = """# --- P17: auto hour threshold ---
from __future__ import annotations
...
def compute_hour_threshold(now: datetime | None = None) -> Dict[str, Any]:
    ...
    for i in range(max(1, lookback_days)):
        ...
        # even if no trades in that hour, we count the day as observed
        days_used += 1
        if c <= 0:
            continue
        h_trades += c
...
def remove_existing_p17(lines: list[str]) -> list[str]:
    ...
    while i < len(lines):
        ...
        i += 1
    if removed:
        print("[P17b] Removed previous P17 block (idempotent).")
    return out
...
def patch_wrapper(ps1: Path) -> None:
    ...
    print(f"[P17b] OK patched: {ps1}")
    print(f"[P17b] Backup: {bkp.name}")
...
def main() -> None:
    ...
    if not ps1.exists():
        raise SystemExit(f"[P17b] Não achei wrapper: {ps1}")
