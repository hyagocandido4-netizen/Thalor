# P9 (v2): Conformal uncertainty gating (CP) + wiring in CLI/observe/tune
...
function Replace-InFile {
  ...
  if ($txt.IndexOf($Old, [StringComparison]::Ordinal) -lt 0) {
...
def train_base_cal_iso_meta(
    ...
    if len(meta_df) >= min_part:
        ...
        # only split if we can keep enough for training and enough for CP calibration
...
def compute_scores(
    ...
    score:
      ...
      - gate_mode=cp  : meta score + abstain via CP (zera score quando rejeita)
    """
    ...
    # gate_mode=cp: apply CP if available, else fallback to meta/iso/conf in that order
    if gate_mode == "cp":
        if (cp is not None) and (meta_score is not None):
            alpha = float(os.getenv("CP_ALPHA", "0.10"))
            ok = cp.accept_mask(meta_score, alpha=alpha)
            out = meta_score.copy()
            out[~ok] = 0.0
            return proba, conf, out, "cp"
        # fallback (don't silently drop to conf if we have meta_score)
...
# paper_pnl_backtest.py (supports P6 options too)
...
# observe_signal_topk_perday.py (gate_mode validation)
Replace-InFile -Path "src/natbin/observe_signal_topk_perday.py" -Old 'if gate_mode not in ("meta", "iso", "conf"):' -New 'if gate_mode not in ("meta", "iso", "conf", "cp"):'
Replace-InFile -Path "src/natbin/observe_signal_topk_perday.py" -Old 'if gate_mode not in ("meta","iso","conf"):' -New 'if gate_mode not in ("meta","iso","conf","cp"):'
Replace-InFile -Path "src/natbin/observe_signal_topk_perday.py" -Old 'if gate_mode not in (\"meta\", \"iso\", \"conf\"):' -New 'if gate_mode not in (\"meta\", \"iso\", \"conf\", \"cp\"):'
...
Write-Host "P9(v2) applied."
Write-Host "Now confirm:"
Write-Host "  .\.venv\Scripts\python.exe -m natbin.paper_pnl_backtest --help | Select-String gate-mode"
Write-Host "Then run:"
Write-Host "  $env:CP_ALPHA='0.10'"
Write-Host "  .\.venv\Scripts\python.exe -m natbin.paper_pnl_backtest --config .\configs\wr_meta_hgb_3x20.yaml --k 1 --holdout-days 60 --payout 0.8 --gate-mode cp --meta-model hgb --thresh-on ev --retrain-every-days 20 --threshold 0.10"
