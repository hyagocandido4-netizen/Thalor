...
# Ensure os is imported (used by env flags)
if ($paper -notmatch "(?m)^import\\s+os\\s*$") {
  if ($paper -notlike "*import argparse*") {
    throw "Anchor não encontrado em ${paperPath}: import argparse"
  }
  $paper = Patch-InsertAfter -Text $paper -Needle "import argparse" -Insert "`nimport os"
}

if ($paper -notmatch "covreg_thresholds\\(") {
  # insert helper after ZoneInfo import
...
# Replace only the threshold field in the row dict (more robust than raw quote escaping)
$obs = [System.Text.RegularExpressions.Regex]::Replace(
  $obs,
  '"threshold"\s*:\s*float\(\s*thr\s*\)',
