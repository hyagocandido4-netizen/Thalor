...
Require-Path "src\natbin\observe_signal_latest.py" "Nao achei src\natbin\observe_signal_latest.py"
Require-Path "observe_loop.ps1" "Nao achei observe_loop.ps1 na raiz"

# -------------------------
# Patch observe_signal_latest.py (CSV retry + fallback + SQLite sempre)
# -------------------------
...
# -------------------------
# Patch observe_loop.ps1: definir LIVE_SIGNALS_PATH diário (idempotente)
# -------------------------
$loopPath = "observe_loop.ps1"
$loop = Get-Content $loopPath -Raw

if ($loop -notmatch "LIVE_SIGNALS_PATH") {
  # insere antes da chamada observe_signal_latest
  $needle = [regex]::Escape('& $py -m natbin.observe_signal_latest')
  if ($loop -notmatch $needle) {
    throw "Nao encontrei a linha que chama natbin.observe_signal_latest em observe_loop.ps1"
  }
  $insert = @'
  # define CSV diário para evitar lock ao abrir em Excel
  $env:LIVE_SIGNALS_PATH = Join-Path (Join-Path (Get-Location) "runs") ("live_signals_{0}.csv" -f (Get-Date -Format "yyyyMMdd"))
'@
  $loop2 = [regex]::Replace($loop, $needle, ($insert + "`r`n  & `$py -m natbin.observe_signal_latest"), 1)
  Write-Utf8NoBomFile $loopPath $loop2
}

Write-Host "Patch aplicado: observe_signal_latest.py (SQLite + CSV retry/fallback) e observe_loop.ps1 (CSV diário)." -ForegroundColor Green
