...
if (-not (Test-Path ".\.venv\Scripts\python.exe")) { throw "Nao achei .venv\Scripts\python.exe" }
if (-not (Test-Path "src\natbin\observe_signal_topk_perday.py")) { throw "Nao achei src\natbin\observe_signal_topk_perday.py" }
...
def _tz(tzname: str):
    # Sempre tenta IANA; se falhar, cai num offset -03:00
    try:
...
def main():
    ...
    # Day do candle (timezone configurada)
    ...
    # ✅ dt_local como TEXTO no horário local do Windows (evita conversão da planilha)
    dt_local_txt = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    row = {
        "dt_local": dt_local_txt,
        "day": str(last_day),
        "ts": int(last_ts),
    ...
    if not ok_csv:
        fb = Path("runs") / f"live_signals_fallback_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{os.getpid()}.csv"
...
Write-Host "OK. dt_local agora será gravado como texto no horario local." -ForegroundColor Green
