...
def _write_signals_db(path: Path, ts_values: list[int]) -> None:
    ...
    try:
        ...
        for idx, ts in enumerate(ts_values[:10]):
        ...
        for idx, ts in enumerate(ts_values[10:140], start=10):
...
def _write_daily_summaries(repo_root: Path) -> None:
    ...
    payloads = [(0, 2, 0.5), (1, 3, 0.67), (2, 1, 1.0), (3, 1, 0.0)]
    for days_back, trades_eval_total, win_rate in payloads:
        ...
        daily_summary_path(day=day, asset=ASSET, interval_sec=INTERVAL, out_dir=runs_dir).write_text(json.dumps(payload, indent=2), encoding='utf-8')
...
def _write_decision_latest(repo_root: Path, ts: int) -> None:
    ...
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps({'kind': 'decision', 'observed_at_utc': datetime.now(tz=UTC).isoformat(timespec='seconds'), 'asset': ASSET, 'interval_sec': INTERVAL, 'day': datetime.now(tz=UTC).strftime('%Y-%m-%d'), 'ts': int(ts), 'action': 'HOLD', 'reason': 'regime_block', 'blockers': 'cp_reject', 'executed_today': 0, 'payout': 0.8, 'ev': -1.0, 'proba_up': 0.49, 'conf': 0.51, 'score': 0.0}, indent=2), encoding='utf-8')


def _write_legacy_mismatch(repo_root: Path) -> None:
    base = portfolio_runs_dir(repo_root)
    base.mkdir(parents=True, exist_ok=True)
    payload = {'cycle_id': 'stale_cycle', 'allocation_id': 'stale_alloc', 'finished_at_utc': '2026-03-11T06:20:34+00:00', 'at_utc': '2026-03-11T06:20:34+00:00', 'config_path': 'config/multi_asset.yaml', 'runtime_profile': 'default', 'candidates': [], 'selected': [], 'suppressed': []}
    (base / 'portfolio_cycle_latest.json').write_text(json.dumps(payload, indent=2), encoding='utf-8')
    (base / 'portfolio_allocation_latest.json').write_text(json.dumps(payload, indent=2), encoding='utf-8')


def main() -> int:
    with tempfile.TemporaryDirectory(prefix='thalor_int_recovery2_') as td:
        root = Path(td)
        cfg = _write_config(root)
        base_ts = 1773000000
        ts_values = [base_ts + (i * INTERVAL) for i in range(140)]
        _write_dataset(root / 'data' / 'dataset_phase2.csv', ts_values)
        _write_daily_summaries(root)
        _write_decision_latest(root, ts_values[-1])
        _write_legacy_mismatch(root)
        ...
        payload = refresh_config_intelligence(repo_root=root, config_path=cfg, asset=ASSET, interval_sec=INTERVAL, rebuild_pack=True, materialize_portfolio=True)
        assert payload['ok'] is True, payload
        assert payload['materialized_portfolio']['ok'] is True, payload
        item = payload['items'][0]
        assert item['pack_training_rows'] >= 100, payload
        assert item['pack_anti_overfit_available'] is True, payload
        assert item['latest_eval_anti_overfit_available'] is True, payload

        status = portfolio_status_payload(repo_root=root, config_path=cfg)
        assert status['latest_cycle'] is not None, status
        assert status['latest_allocation'] is not None, status
        assert status['latest_sources']['cycle']['source'] == 'scoped', status
        assert status['latest_sources']['allocation']['source'] == 'scoped', status

        surf = intelligence_payload(repo_root=root, config_path=cfg)
        assert 'portfolio_artifact_scope' not in list(surf.get('warnings') or []), surf
        assert ((surf.get('summary') or {}).get('anti_overfit') or {}).get('available') is True, surf

    print('intelligence_recovery_2_smoke: OK')
    return 0
