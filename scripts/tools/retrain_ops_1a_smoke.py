...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix='thalor_retrain_ops_1a_') as td:
        root = Path(td)
        ...
        with patch('natbin.ops.retrain_ops.refresh_config_intelligence', side_effect=fake_refresh), \
             patch('natbin.ops.retrain_ops._now_dt', return_value=datetime(2026, 3, 22, 23, 41, tzinfo=UTC)), \
             patch('natbin.ops.retrain_ops._now_iso', return_value='2026-03-22T23:41:00+00:00'):
        ...
        assert status_payload['items'][0]['status']['state'] == 'queued', status_payload
        assert status_payload['items'][0]['cooldown_refresh']['normalized'] is True, status_payload
        item = run_payload['items'][0]
        assert item['executed'] is True, item
        ...
        assert review['executed'] is True, review
        assert len(calls) >= 2 and calls[0]['rebuild_pack'] is False and calls[1]['rebuild_pack'] is True, calls

    print('retrain_ops_1a_smoke: OK')
    return 0
