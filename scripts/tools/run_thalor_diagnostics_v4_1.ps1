...
# Reviewed for Windows PowerShell 5.1 and PowerShell 7 JSON parsing / UTF-8 no-BOM artifact writing
...
function Try-ParseJsonCandidate {
    param([string]$Candidate)

    try {
        return @{
            Parsed     = (ConvertFrom-JsonCompat -JsonText $Candidate)
            Raw        = $Candidate
            ParseError = ''
        }
    }
...
function Try-ParseJsonFromText {
    ...
    $direct = Try-ParseJsonCandidate -Candidate $candidate
    if ($null -ne $direct.Parsed) {
        return $direct
    }

    $lastError = $direct.ParseError
    $pairs = @(
        @{ Start = '{'; End = '}' },
        @{ Start = '['; End = ']' }
    )
    ...
    foreach ($pair in $pairs) {
        ...
        if ($start -ge 0 -and $finish -gt $start) {
            ...
            $attempt = Try-ParseJsonCandidate -Candidate $slice
            if ($null -ne $attempt.Parsed) {
                return $attempt
            }
...
function Get-SemanticParseErrorValue {
    param($SemanticAssessment)
    if ($null -eq $SemanticAssessment) { return '' }
    if ($SemanticAssessment.PSObject.Properties.Match('parse_error').Count -gt 0) {
        return [string]$SemanticAssessment.parse_error
    }
    return ''
}
...
function New-StepContext {
    ...
    Write-Utf8NoBomFile -Path $commandFile -Content $CommandText
    Write-JsonNoBomFile -Path (Join-Path $ctxDir 'env_scope.json') -Object (Get-RedactedEnvSummary) -Depth 8
...
function Finalize-Step {
    ...
    $meta = [pscustomobject]@{
        ...
        semantic_parse_error = $SemanticParseError
        exit_code          = $ExitCode
        timeout            = $TimeoutSec
        timed_out          = $TimedOut
    ...
    Write-JsonNoBomFile -Path $Context.metaFile -Object $meta -Depth 8
...
function Write-SkippedStep {
    ...
    Write-Utf8NoBomFile -Path $ctx.combinedLog -Content $Reason
...
function Invoke-NativeStep {
    ...
    try {
        ...
        Write-Utf8NoBomFile -Path $ctx.stderrFile -Content ($_ | Out-String)
        $exitCode = 1
    }
    ...
    Write-Utf8NoBomFile -Path $ctx.combinedLog -Content $combined
    ...
    if ($SemanticProfile -like 'json*') {
        ...
        if ($null -ne $parsed -and $null -ne $parsed.Parsed) {
            $semantic = Evaluate-JsonSemantics -JsonObject $parsed.Parsed -Profile $SemanticProfile
            Write-Utf8NoBomFile -Path (Join-Path $ctx.ctxDir 'stdout.json') -Content $parsed.Raw
        }
        else {
            $parseError = if ($null -ne $parsed) { [string]$parsed.ParseError } else { '' }
            $reasons = New-Object System.Collections.ArrayList
            [void]$reasons.Add('json_output_not_parseable')
            if (-not [string]::IsNullOrWhiteSpace($parseError)) {
                [void]$reasons.Add(('json_parse_error=' + $parseError))
                Write-Utf8NoBomFile -Path (Join-Path $ctx.ctxDir 'json_parse_error.txt') -Content $parseError
            }
            if ($null -ne $parsed -and -not [string]::IsNullOrWhiteSpace([string]$parsed.Raw)) {
                Write-Utf8NoBomFile -Path (Join-Path $ctx.ctxDir 'json_parse_candidate.txt') -Content ([string]$parsed.Raw)
            }
            $semantic = New-SemanticAssessment -Status 'warn' -Reasons @($reasons) -Detected $true -ParseError $parseError
        }
    }
    ...
    Finalize-Step -Context $ctx -FinalStatus $finalStatus -Optional $Optional -ExitCode $exitCode -ProcessStatus $processStatus -SemanticStatus $semantic.status -SemanticReasons $semantic.reasons -SemanticParseError (Get-SemanticParseErrorValue $semantic) -TimeoutSec $TimeoutSec -TimedOut $timedOut -TrackRoots $TrackRoots -AlwaysInclude $AlwaysInclude -SemanticProfile $SemanticProfile
}
...
function Invoke-PowerShellStep {
    ...
    try {
        ...
        foreach ($item in @($output)) {
            ...
            [void]$captured.Add(($item | Out-String).TrimEnd())
        }
        ...
        [void]$captured.Add(($_ | Out-String).TrimEnd())
        $exitCode = 1
    }
    finally {
        Pop-Location
    ...
    $combined = [string]::Join([Environment]::NewLine, @($captured))
    Write-Utf8NoBomFile -Path $ctx.combinedLog -Content $combined
    ...
    Finalize-Step -Context $ctx -FinalStatus $finalStatus -Optional $Optional -ExitCode $exitCode -ProcessStatus $processStatus -SemanticStatus 'ok' -SemanticReasons @() -SemanticParseError '' -TimeoutSec 0 -TimedOut $false -TrackRoots $TrackRoots -AlwaysInclude $AlwaysInclude -SemanticProfile 'none'
}
...
function Write-SummaryFile {
    ...
    Write-JsonNoBomFile -Path $summaryPath -Object $payload -Depth 10
    Write-Utf8NoBomFile -Path (Join-Path $ArtifactsParent 'LATEST_SESSION.txt') -Content $ArtifactsRoot
