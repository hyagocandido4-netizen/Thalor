...
Write-Host "== Phase 3.2 Tuner (HGB + filter + threshold) ==" -ForegroundColor Cyan

Write-Utf8NoBomFile "src\natbin\paper_tune_v2.py" @'
...
Write-Host "paper_tune_v2.py criado." -ForegroundColor Green
...
if (-not $NoRun) {
  Write-Host "`nRodando TUNER..." -ForegroundColor Cyan
  & $py -m natbin.paper_tune_v2
  if ($LASTEXITCODE -ne 0) { throw "paper_tune_v2 falhou" }

  Write-Host "`nPhase 3.2 concluida." -ForegroundColor Green
} else {
  Write-Host "`nGerado. Para rodar:" -ForegroundColor Yellow
  Write-Host "  .\.venv\Scripts\python.exe -m natbin.paper_tune_v2"
}