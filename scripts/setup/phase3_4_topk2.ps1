...
Write-Host "== Phase 3.4 (Top-K por sessao) ==" -ForegroundColor Cyan
Write-Host "K = $K" -ForegroundColor Cyan

Write-Utf8NoBomFile "src\natbin\paper_topk_multiwindow.py" @'
...
# roda
$env:TOPK_K = "$K"
& $py -m natbin.paper_topk_multiwindow
if ($LASTEXITCODE -ne 0) { throw "paper_topk_multiwindow falhou" }

Write-Host "`nOK. Veja runs/topk2_multiwindow_*/summary.json" -ForegroundColor Green