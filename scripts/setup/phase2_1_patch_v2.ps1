...
Require-Path "config.yaml" "Nao encontrei config.yaml. Rode este script na raiz do projeto."
...
Write-Host "== Phase 2.1 Patch v2 ==" -ForegroundColor Cyan

# -------------------------
# (1) Patch config.yaml: thresholds mais baixos
# -------------------------
$pyPatchConfig = @'
import yaml
...
# -------------------------
# (2) Patch dataset2.py: adiciona SMA/BB/MACD (idempotente)
# -------------------------
$datasetPath = "src\natbin\dataset2.py"
...
# -------------------------
# (3) Patch train_walkforward.py: insere seleção Phase 2.1 antes do 'stamp =' (idempotente)
# -------------------------
$trainPath = "src\natbin\train_walkforward.py"
$tw = Read-FileRaw $trainPath

if ($tw -match "best_accuracy_cov0\.1-2%_min300trades" -or $tw -match "MIN_TRADES\s*=\s*300") {
  Write-Host "train_walkforward.py: regra Phase 2.1 ja presente. (skip)" -ForegroundColor DarkGray
} else {
  $pattern = "(?m)^(?<indent>\s*)stamp\s*=\s*datetime\.now\(\)\.strftime\(.+\)\s*$"

  if ($tw -notmatch $pattern) {
    throw "train_walkforward.py: nao achei a linha 'stamp = datetime.now().strftime(...)' para inserir o patch. Abra o arquivo e me mande as linhas onde cria 'stamp'/'run_dir'."
  }

  $tw2 = [regex]::Replace($tw, $pattern, {
      param($m)
      $indent = $m.Groups["indent"].Value

      $block = @"
$indent# --- Phase 2.1 selection: poucos sinais, mas mensurável ---
$indent COV_MIN = 0.001   # 0.10%
$indent COV_MAX = 0.02    # 2.00%
$indent MIN_TRADES = 300  # mínimo de trades totais (somando folds)

$indent candidates = thr_df[
$indent     (thr_df["coverage"] >= COV_MIN) &
$indent     (thr_df["coverage"] <= COV_MAX) &
$indent     (thr_df["taken"] >= MIN_TRADES)
$indent ].copy()

$indent if candidates.empty:
$indent     candidates2 = thr_df[thr_df["taken"] >= 50].copy()
$indent     if candidates2.empty:
$indent         best = thr_df.iloc[0]
$indent         rule = "fallback_best_overall"
$indent     else:
$indent         best = candidates2.iloc[0]
$indent         rule = "fallback_best_accuracy_min50trades"
$indent else:
$indent     best = candidates.iloc[0]
$indent     rule = "best_accuracy_cov0.1-2%_min300trades"
...
"@
  ...
  Write-Host "train_walkforward.py: patch aplicado (inserção segura Phase 2.1)." -ForegroundColor Green
}
