...
Require-Path "data/dataset_phase2.csv" "Nao achei dataset. Rode: python -m natbin.make_dataset"
Require-Path "src/natbin/paper_backtest.py" "Nao achei paper_backtest.py. Rode a fase 3 (paper) primeiro."

Write-Host "== Phase 3.1 Upgrade ==" -ForegroundColor Cyan

# (1) Cria um novo módulo: paper_backtest_v2 (com HGB + regime filter)
Write-Utf8NoBomFile "src\natbin\paper_backtest_v2.py" @'
...
Write-Host "paper_backtest_v2.py criado." -ForegroundColor Green
...
if (-not $NoRun) {
  Write-Host "`nRodando PAPER V2..." -ForegroundColor Cyan
  & $py -m natbin.paper_backtest_v2
  if ($LASTEXITCODE -ne 0) { throw "paper_backtest_v2 falhou" }

  Write-Host "`nPhase 3.1 concluida." -ForegroundColor Green
} else {
  Write-Host "`nGerado. Para rodar:" -ForegroundColor Yellow
  Write-Host "  .\.venv\Scripts\python.exe -m natbin.paper_backtest_v2"
}