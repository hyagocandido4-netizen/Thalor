...
def _resolved_config_or_none(*, config_path: str | Path | None = None, repo_root: str | Path | None = None):
    path = resolve_config_path(repo_root=repo_root, config_path=config_path)
    try:
        from ..config.loader import load_resolved_config

        return load_resolved_config(config_path=path, repo_root=resolve_repo_root(repo_root=repo_root, config_path=path)), path
    except Exception:
        return None, path


def repo_asset(*, config_path: str | Path | None = None, repo_root: str | Path | None = None, default: str = 'UNKNOWN') -> str:
    resolved, path = _resolved_config_or_none(config_path=config_path, repo_root=repo_root)
    if resolved is not None:
        asset = str(getattr(resolved, 'asset', default)).strip()
...
def repo_interval_sec(*, config_path: str | Path | None = None, repo_root: str | Path | None = None, default: int = 300) -> int:
    resolved, path = _resolved_config_or_none(config_path=config_path, repo_root=repo_root)
    if resolved is not None:
        try:
            return int(getattr(resolved, 'interval_sec', default))
        except Exception:
            return int(default)
    p = Path(path)
    if not p.exists():
...
def repo_timezone(*, config_path: str | Path | None = None, repo_root: str | Path | None = None, default: str = 'UTC') -> ZoneInfo:
    resolved, path = _resolved_config_or_none(config_path=config_path, repo_root=repo_root)
    tz_name = default
    if resolved is not None:
        tz_name = str(getattr(resolved, 'timezone', default)).strip() or default
    else:
        p = Path(path)
        if p.exists():
            try:
...
def repo_timezone_name(*, config_path: str | Path | None = None, repo_root: str | Path | None = None, default: str = 'UTC') -> str:
    tz = repo_timezone(config_path=config_path, repo_root=repo_root, default=default)
    return str(getattr(tz, 'key', str(tz)) or default)


def repo_now(*, config_path: str | Path | None = None, repo_root: str | Path | None = None, default_tz: str = 'UTC') -> datetime:
    return datetime.now(tz=repo_timezone(config_path=config_path, repo_root=repo_root, default=default_tz))
