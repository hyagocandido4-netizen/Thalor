...
def build_coverage_profile(
    ...
    *,
    ...
    target_curve: dict[str, float] = {}
    running = 0.0
    for h in hours:
        running += float(trades_by_hour[h])
        observed_frac = (running / total_trades) if total_trades > 0 else ((int(h) + 1) / 24.0)
        cumulative_share[h] = float(observed_frac)
        target_curve[h] = float(max(0.0, min(1.0, observed_frac ** float(curve_power))))

    return {
        'kind': 'coverage_profile',
        'schema_version': 'phase1-coverage-profile-v2',
        'days_used': int(total_days),
        'trades_per_day_observed': float(trades_per_day),
        'target_trades_per_day': float(target),
        'curve_power': float(curve_power),
        'hourly_trade_share': {
            h: ((float(trades_by_hour[h]) / float(total_trades)) if total_trades > 0 else (1.0 / 24.0))
            for h in hours
        },
        'cumulative_trade_share': cumulative_share,
        'target_curve_share': target_curve,
    }
...
def coverage_bias(
    ...
    *,
    ...
    if isinstance(profile, dict):
        ...
        target_curve = dict(profile.get('target_curve_share') or {})
    observed_frac = _safe_float(cumulative.get(hour), ((int(hour) + 1) / 24.0))
    target_frac = _safe_float(target_curve.get(hour), observed_frac ** float(curve_power))
    target_now = float(target_trades_per_day) * float(target_frac)
    delta = float(target_now) - float(executed_today)

    bias = 0.0
    pressure = 'balanced'
    if delta > float(tolerance):
        pressure = 'under_target'
        bias = min(float(delta) * float(bias_weight), float(max_bonus))
    elif delta < -float(tolerance):
        pressure = 'over_target'
        bias = max(float(delta) * float(bias_weight), -float(max_penalty))

    return {
        'hour': hour,
        'observed_cumulative_share': float(observed_frac),
        'target_cumulative_share': float(target_frac),
