...
Package K extracted the quota/pacing policy into a pure Python module so the
Python-native daemon could reason about day budget and next wake-up time.

Package N extends this module to optionally use the execution ledger as the
source of truth for quota consumption while keeping the legacy signal/executed
state path available for disabled execution mode.
"""
...
from ..state.repos import RuntimeTradeLedger
from ..state.summary_paths import repo_timezone_name
...
EXECUTION_PENDING_UNKNOWN = 'execution_pending_unknown'
EXECUTION_OPEN_POSITION = 'execution_open_position'
...
def build_quota_snapshot(
    ...
    *,
    ...
    resolved_config_path = resolve_config_path(repo_root=repo_root, config_path=config_path)
    rcfg = load_resolved_config(config_path=resolved_config_path, repo_root=repo_root)
    tz_name = str(getattr(rcfg, 'timezone', None) or repo_timezone_name(config_path=str(resolved_config_path), repo_root=repo_root))
    local_now, day, sec = compute_quota_day_context(tz_name=tz_name, now_utc=now_utc)
    if pacing_enabled is None:
        pacing_enabled = bool(getattr(rcfg.decision, 'pacing_enable', False)) or _env_truthy('TOPK_PACING_ENABLE', default=False)
    k = max(1, int(topk))

    execution_enabled = bool(getattr(rcfg.execution, 'enabled', False)) and str(getattr(rcfg.execution, 'mode', 'disabled')) != 'disabled'
    source = 'legacy'
    pending_unknown = 0
    open_positions = 0
    if execution_enabled:
        exec_repo = ExecutionRepository(repo_root / 'runs' / 'runtime_execution.sqlite3')
        executed = exec_repo.count_consuming_intents(asset=rcfg.asset, interval_sec=rcfg.interval_sec, day=day)
        pending_unknown = exec_repo.count_pending_unknown(asset=rcfg.asset, interval_sec=rcfg.interval_sec)
        open_positions = exec_repo.count_open_positions(asset=rcfg.asset, interval_sec=rcfg.interval_sec)
        source = 'execution'
    else:
        ledger = RuntimeTradeLedger(
            signals_db=repo_root / 'runs' / 'live_signals.sqlite3',
            state_db=repo_root / 'runs' / 'live_topk_state.sqlite3',
            default_interval=rcfg.interval_sec,
        )
        executed = ledger.executed_today_count(rcfg.asset, rcfg.interval_sec, day)
    ...
    if execution_enabled and pending_unknown >= max(1, int(getattr(rcfg.execution.limits, 'max_pending_unknown', 1))):
        kind = EXECUTION_PENDING_UNKNOWN
        poll = max(1, int(getattr(rcfg.execution.reconcile, 'poll_interval_sec', 5)))
        wake_utc = (now_utc or datetime.now(UTC)).astimezone(UTC) + timedelta(seconds=poll)
        next_at, next_wake_utc, sleep_sec = '', _fmt_utc(wake_utc), int(poll)
        budget_left_now = 0
    elif execution_enabled and open_positions >= max(1, int(getattr(rcfg.execution.limits, 'max_open_positions', 1))):
        kind = EXECUTION_OPEN_POSITION
        poll = max(1, int(getattr(rcfg.execution.reconcile, 'poll_interval_sec', 5)))
        wake_utc = (now_utc or datetime.now(UTC)).astimezone(UTC) + timedelta(seconds=poll)
        next_at, next_wake_utc, sleep_sec = '', _fmt_utc(wake_utc), int(poll)
        budget_left_now = 0
