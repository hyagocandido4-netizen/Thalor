...
This module is intentionally additive in Package I: it does not replace the
current PowerShell loops yet. It provides a Python-native representation of a
single auto-loop cycle so future packages can thin the shell layer without
changing the runtime behaviour abruptly.
"""
...
def _pwsh_step(repo_root: Path, name: str, script_rel: str, extra_args: Iterable[str], timeout_env: str, timeout_default: int) -> StepCommand:
    timeout_sec = _as_int(os.getenv(timeout_env), timeout_default)
    pwsh = os.getenv("PWSH_EXE") or "pwsh"
    script = str((repo_root / script_rel).resolve())
    argv = [pwsh, "-ExecutionPolicy", "Bypass", "-File", script, *extra_args]
    return StepCommand(name=name, argv=argv, timeout_sec=timeout_sec, cwd=str(repo_root))
...
def build_auto_cycle_plan(repo_root: Path | str, *, topk: int = 3, lookback_candles: int = 2000) -> list[StepCommand]:
    ...
    extra_args = ["-Once", "-SkipCollect", "-SkipDataset", "-LookbackCandles", str(int(lookback_candles))]
    if int(topk) > 0:
